...
void PrintInterfaces(TPZCompMesh *cmesh,std::ostream &out) {
  ...
  for(i=0;i<nelem;i++) {
    ...
    interface = (TPZInterfaceElement *)cel;
		countinterf++;
    out << "INTERF " << i << "  Left " << interface->LeftElement()->Index();
    out << "  Right " << interface->RightElement()->Index() << std::endl;
  }
