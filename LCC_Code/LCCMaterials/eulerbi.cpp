...
TEulerLaw2D::TEulerLaw2D(int id) : TConservationLaw(id) {
  fGamma=1.4;
}
...
int TEulerLaw2D::VariableIndex(const std::string &name) {
  if(name == "density") return 0;
  else if(name == "velocity_x") return 1;
  else if(name == "velocity_y") return 2;
  else if(name == "energy") return 3;
  else if(name == "Pressure") return 4;
  else if(name == "enthalpy") return 5;
  else if(name == "dens_velocity_x") return 6;
  else if(name == "dens_velocity_y") return 7;
  else if(name == "dens_energy") return 8;
...
void TEulerLaw2D::Solution(TPZVec<STATE> &Sol,TPZFMatrix<STATE> &DSol,TPZFMatrix<REAL> &axes,int var,TPZVec<STATE> &Solout) {
...
STATE TEulerLaw2D::Pressure(TPZVec<STATE> &U) {
...
void TEulerLaw2D::FunctionFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &flux) {
...
//      ReAssembling = 1;
      flux(0,0) = flux(2,0) = flux(3,0) = flux(0,1) = flux(1,1) = flux(3,1) = 0.;
//      flux[1] = flux[6] = 0.;
	  std::cout << "EulerLaw2D::Flux find nule density.\n";
      flux(1,0) = flux(2,1) = pressao;  // Jorge 17/08
      return;
  }
  flux(0,0) = U[1];
  flux(1,0) = ((U[1]/U[0])*U[1]) + pressao;
  flux(2,0) = U[1]*(U[2]/U[0]);
  flux(3,0) = (U[1]/U[0])*(U[3]+pressao);
  flux(0,1) = U[2];
  flux(1,1) = U[2]*(U[1]/U[0]);
  flux(2,1) = ((U[2]/U[0])*U[2]) + pressao;
  flux(3,1) = (U[2]/U[0])*(U[3] + pressao);
}
...
void TEulerLaw2D::JacobFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &jacob) {
...
//    if(!IsZero(U[1])) {
  ...
  jacob(0,0)=jacob(0,2)=jacob(0,3)=jacob(2,3)=jacob(0,4)=jacob(0,5)=jacob(0,7)=jacob(1,7)=0.;
  jacob(0,1)=jacob(0,6)=1.;
  jacob(1,3)=jacob(2,7)=gammam1;
  jacob(1,1)=(3.-fGamma)*velx;
  jacob(2,6)=(3.-fGamma)*vely;
  jacob(1,2)=-1.*gammam1*vely;
  jacob(2,5)=-1.*gammam1*velx;
  jacob(3,2)=jacob(3,5)=jacob(1,2)*velx;
  jacob(2,0)=jacob(1,4)=-1.*velx*vely;
  jacob(2,1)=jacob(1,5)=vely;
  jacob(2,2)=jacob(1,6)=velx;
  jacob(3,3)=fGamma*velx;
  jacob(3,7)=fGamma*vely;
  aux = gammam1*(velx*velx+vely*vely);
  jacob(1,0) = 0.5*aux - (velx*velx);
  jacob(3,1) = (fGamma*ener) - (.5*gammam1*(3*velx*velx+vely*vely));
  jacob(3,6) = (fGamma*ener) - (.5*gammam1*(velx*velx+3*vely*vely));
  jacob(2,4) = 0.5*aux - (vely*vely);
  jacob(3,0) = (aux-(fGamma*ener))*velx;
  jacob(3,4) = (aux-(fGamma*ener))*vely;
}

void TEulerLaw2D::JacobFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &jacob,TPZVec<REAL> &normal) {
#ifndef NOTDEBUG
  if(IsZero(U[0])) {
...
STATE TEulerLaw2D::MaxEigJacob(TPZVec<STATE> &U,TPZVec<REAL> &normal) {
  STATE rho = U[0];
...
STATE TEulerLaw2D::ValEigJacob(TPZVec<STATE> &U,int order,int dim) {
  if(IsZero(U[0])) return 0.;
  STATE velx = U[1]/U[0];
  if(dim==1) {
    if(!order || order==1) return velx;
    STATE velson = (fGamma*Pressure(U))/U[0];
...
void TEulerLaw2D::ValRoeMatrix(TPZVec<STATE> &u,TPZVec<STATE> &Up1,TPZFMatrix<STATE> &ValRoe) {
}
void TEulerLaw2D::RoeMatrix(TPZVec<STATE> &U,TPZVec<STATE> &Up1,TPZFMatrix<STATE> &Roe) {
}

void TEulerLaw2D::EigRoeMatrix(TPZVec<STATE> &U,TPZVec<STATE> &Up1,TPZVec<STATE> &Roe) {
...
void TEulerLaw2D::ContributeBCInterface(TPZMaterialData &data, TPZMaterialData &dataleft, REAL weight, TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef,TPZBndCond &bc) {
    ...
    switch (bc.Type()) {
        case 0 :            // Dirichlet condition
            for(in = 0 ; in < phr; in++) {
                for(cn=0;cn<nvar;cn++)
                    ef(nvar*in+cn,0) += (STATE)(gBigNumber* phi(in,0) * weight) * fDeltaT*fCoef* v2[cn];
                for (jn = 0 ; jn < phr; jn++) {
                    for(cn=0;cn<nvar;cn++)
        ...
        {
            STATE press = Pressure(u);
            v2[1] = data.normal[0]*press;
            v2[2] = data.normal[1]*press;
        ...
        case 2 :        // mixed condition
        {
            for(in = 0 ; in < phi.Rows(); in++) {
                for(cn=0;cn<nvar;cn++)
                    ef(nvar*in+cn, 0) += v2[cn] * (STATE)(phi(in, 0) * weight*fDeltaT*fCoef);
                for (jn = 0 ; jn < phi.Rows(); jn++) {
                    for(cn=0;cn<nvar;cn++)
                        for(kn=0;kn<nvar;kn++)   // Acho que esto no funciona
                            ek(nvar*in+cn,nvar*jn+kn) += bc.Val1()(cn,kn) * (STATE)(phi(in,0) * phi(jn,0) * weight*fDeltaT*fCoef);     // peso de contorno => integral de contorno
                }
            }
        }
            break;
        case 3:     // outflow condition
            {
                STATE press = Pressure(u);
                STATE velxnorm = ((u[1]/u[0])*data.normal[0])+((u[2]/u[0])*data.normal[1]);
                v2[0] = velxnorm*u[0];
                v2[1] = velxnorm*u[1] + data.normal[0]*press;
                v2[2] = velxnorm*u[2] + data.normal[1]*press;
                v2[3] = velxnorm*(u[3] + press);
                for(in = 0 ; in < phi.Rows(); in++) {
                    for(jn=0;jn<nvar;jn++)
                        ef(nvar*in+jn,0) -= v2[jn] * (STATE)(phi(in,0) * weight*fDeltaT*fCoef);
                }
...
void TEulerLaw2D::ContributeBC(TPZMaterialData &data, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef, TPZBndCond &bc) {
        
    TPZFMatrix<REAL>  &phi = data.phi;
    TPZVec<STATE> &u = data.sol[0];
    int phr = phi.Rows();
    int nvar = bc.Val2().Rows();
    short in,jn,cn,kn;
    TPZManVector<STATE> v2(nvar,0.);
...
void TEulerLaw2D::ContributeOverInterface(TPZVec<REAL> &x,TPZVec<STATE> &u,TPZVec<STATE> &up1,
     REAL weight,REAL area,int type,TPZFMatrix<REAL> &axes,TPZFMatrix<REAL> &phi0,TPZFMatrix<REAL> &phi1,TPZVec<REAL> &normal,
...
//  else {
    STATE maxeigen = MaxEigJacob(u,normal);
...
#endif
  int i, j, c;
    TPZFMatrix<STATE> flux1(nvar,dim,0.);
  TPZFMatrix<STATE> flux2(nvar,dim,0.);
  TPZVec<REAL> flux(nvar,0.);
  double aux,alfa;
  FunctionFlux(u,flux1);
  FunctionFlux(up1,flux2);
  for(i=0;i<nvar;i++) {
    flux[i] = 0.;
    for(j=0;j<dim;j++)
      flux[i] += normal[j]*(flux1(i,j)+flux2(i,j));
  }

  alfa = MaxEigJacob(u,normal);
  aux = MaxEigJacob(up1,normal);
...
int TEulerLaw2D::ValJacobFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &valjacob,TPZVec<REAL> &normal) {
	valjacob.Zero();
  STATE rho = U[0];
...
/******************************   **************************************/
void TEulerLaw2D::JacobFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &Ajacob,TPZFMatrix<STATE> &Bjacob) {
  if(IsZero(U[0])) {
	  std::cout << "\nERRO : Densidade NULA. Falla jacobiano.\n";
      exit(1);
  }
...
void TEulerLaw2D::InvJacob2d(TPZFMatrix<REAL> &axes,TPZFMatrix<REAL> &jacinv) {
...
void TEulerLaw2D::MatrixDiff(TPZVec<STATE> &sol,TPZFMatrix<REAL> &axes, TPZFMatrix<REAL> &jacinv,TPZFMatrix<STATE>
