...
TEulerLaw1D::TEulerLaw1D(int id) : TConservationLaw(id) {
  fGamma=1.4;
}
...
int TEulerLaw1D::VariableIndex(const std::string &name) {
  if(name == "density") return 0;
  else if(name == "dens_velocity") return 1;
  else if(name == "dens_energy") return 2;
  else if(name == "velocity_x") return 4;
  else if(name == "energy") return 5;
  else if(name == "Pressure") return 3;
  else if(name == "enthalpy") return 6;
...
void TEulerLaw1D::Solution(TPZVec<STATE> &Sol,TPZFMatrix<STATE> &DSol,TPZFMatrix<REAL> &axes,
...
STATE TEulerLaw1D::Pressure(TPZVec<STATE> &U) {
  if(IsZero(U[0])) return (fGamma-1.)*U[2];
  return (fGamma-1.)*(U[2]-(.5*U[1]*(U[1]/U[0])));
}

void TEulerLaw1D::FunctionFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &flux) {
...
/*  if((2*U[0]*U[2])<(U[1]*U[1])) {
  ...
  flux(0,0)=U[1];
  flux(0,1)=(U[1]*velocidade)+pressao;
  flux(0,2)=velocidade*(U[2]+pressao);
}

void TEulerLaw1D::JacobFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &jacob) {
  if(IsZero(U[0])) {
//    if(!IsZero(U[1])) {
...
STATE TEulerLaw1D::MaxEigJacob(TPZVec<STATE> &U,TPZVec<REAL> &normal) {
	REAL vel = 0., velson = 0.;
	if(!IsZero(U[0])) {
...
STATE TEulerLaw1D::ValEigJacob(TPZVec<STATE> &u,int order,int dim) {
#ifndef NOTDEBUG
...
void TEulerLaw1D::JacobFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &jacob,TPZVec<REAL> &normal) {
#ifndef NOTDEBUG
  if(IsZero(U[0])) {
...
int TEulerLaw1D::ValJacobFlux(TPZVec<STATE> &U,TPZFMatrix<STATE> &valjacob,TPZVec<REAL> &normal) {
	valjacob.Zero();
  REAL rho = U[0];
...
void TEulerLaw1D::ValRoeMatrix(TPZVec<STATE> &u,TPZVec<STATE> &Up1,TPZFMatrix<STATE> &ValRoe) {
}
void TEulerLaw1D::RoeMatrix(TPZVec<STATE> &U,TPZVec<STATE> &Up1,TPZFMatrix<STATE> &Roe) {
}

//void TEulerLaw1D::JacobFluxInv_Diffussion(TPZVec<REAL> &U,TPZVec<REAL> &coef,TPZFMatrix &jacob) {
//}

void TEulerLaw1D::EigRoeMatrix(TPZVec<STATE> &U,TPZVec<STATE> &Up1,TPZVec<STATE> &Roe) {
...
void TEulerLaw1D::Print(std::ostream &out) {
    ...
    char density[256];
    strncpy(density,"densidad.eul",255);
	char velocity[256];
    strncpy(velocity,"velocity.eul",255);
	char energy[256];
    strncpy(energy,"energy.eul",255);
	output1 = fopen(density,"w");
...
void TEulerLaw1D::ContributeOverInterface(TPZVec<REAL> &x,TPZVec<STATE> &u,TPZVec<STATE> &up1,
